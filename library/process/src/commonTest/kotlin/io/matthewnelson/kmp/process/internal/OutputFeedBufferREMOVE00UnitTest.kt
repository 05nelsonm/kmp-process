...
class OutputFeedBufferREMOVE00UnitTest {
    ...
    fun givenOnData_whenLenLessThan1_thenIgnores() {
        val feed = OutputFeedBufferREMOVE00.of(maxSize = 5)
        val buf = ReadBuffer.allocate()
    ...
    fun givenMaxSize_whenOutputExceeded_thenIgnoresFurtherOutput() {
        val feed = OutputFeedBufferREMOVE00.of(maxSize = 10)
        val buf = ReadBuffer.allocate()
    ...
    fun givenNoOutput_whenDoFinal_thenReturnsEmptyData() {
        val data = OutputFeedBufferREMOVE00.of(maxSize = 2).doFinal()
        assertEquals(0, data.size)
